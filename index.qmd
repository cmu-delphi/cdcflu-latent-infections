...
## From reported incident cases to incident infection onset
...
latent_infections |>
  ...
  ggplot(aes(date)) +
  geom_ribbon(aes(
    ymin = pmax(infections - qnorm(.95) * infections_sd, cases), 
    ymax = pmax(infections + qnorm(.95) * infections_sd, cases)), 
    fill = tertiary, alpha = .2
  ) +
  geom_ribbon(aes(
    ymin = pmax(infections - qnorm(.9) * infections_sd, cases), 
    ymax = pmax(infections + qnorm(.9) * infections_sd, cases)), 
    fill = tertiary, alpha = .3
  ) +
  geom_ribbon(aes(
    ymin = pmax(infections - qnorm(.75) * infections_sd, cases), 
    ymax = pmax(infections + qnorm(.75) * infections_sd, cases)), 
    fill = tertiary, alpha = .5
  ) +
